...
if __name__ == '__main__':
    ...
    if vote:
        ...
        sample = np.random.choice(indices, size=min(sample_size, len(indices)), replace=False)
        train_docs = [train_docs[i] for i in sample]
        train_labels = [train_labels[i] for i in sample]

        indices_2 = np.arange(len(train_docs_2))
        sample_2 = np.random.choice(indices_2, size=min(sample_size, len(indices_2)), replace=False)
